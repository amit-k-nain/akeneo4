...
class BaseWriter extends \AbstractItemMediaWriter implements
    \ItemWriterInterface,
    ...
    \StepExecutionAwareInterface
{
    ...
    {
        $flatItems = [];
        foreach ($items as $item) { dump($item); die('writer');
            $prodHeader = [];
        ...
        foreach ($product as $key => $value) {
            ...
            if (is_array($value) && sizeof($value) > 1) {
                while ($value) {
                    ...
                    foreach ($versiosData as $key => $data) {
                        ...
                        // set field
                        $formatterItem['Identifier'] = $identifier;
                        $formatterItem['Entity'] = $versiosData['entity'];
                        $formatterItem['Version'] = $versiosData['version'];
                        $formatterItem['Author'] = $versiosData['author'];
                        $formatterItem['Logged At'] = $versiosData['logged_at'];
                        $formatterItem['Field'] = $label;
                        ...
                        if (sizeof($attrKeyData) > 1 && in_array($attrKeyData[0], $keys)) {
                            ...
                            $formatterItem['Old Value'] = strip_tags($oldData.' '.$data['old']);
                            $formatterItem['New Value'] = strip_tags($oldData.' '.$data['new']);
                        ...
                        if (is_array($data) && (sizeof($data) > 1)) {
                            $formatterItem['Old Value'] = strip_tags($data['old']);
                            $formatterItem['New Value'] = strip_tags($data['new']);
                        } else {
            ...
            // product version history code for the first time product create
                ...
                foreach ($value as $key => $data) {
                    ...
                    $formatterItem['Version'] = $value['version'];
                    $formatterItem['Author'] = $value['author'];
